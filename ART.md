...
- Язык промпта — **English**; формат — **вертикальный портрет 2:3** (в ChatGPT/gpt-image
  выбрать размер **`1024×1536`**, не дефолтный квадрат); поле `art` — English.
- Без текста и рамки — рамку/числа рисует клиент (DESIGN §9).
- **Куда сохранять:** `client-godot/art/<id>.png` (имя файла = `id` карты). Это
  клиентский ассет (Godot читает его как `res://art/<id>.png`); сервер про арт не знает.
  После добавления новых PNG один раз открыть проект в Godot (или
  `Godot --headless --path client-godot --import`), чтобы движок их заимпортил.
