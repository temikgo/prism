...
const KW := {
	...
	"reflux": "Отсвет N: каждое ваше исцеление бьёт вражеского героя на N.",
	"requiem": "Реквием N: когда умирает другое ваше существо — N урона вражескому герою и +N HP вашему герою.",
	...
	"bulwark": "Зарево N: в начале вашего хода вашему герою +N зарева. Зарево выгорает под уроном раньше HP.",
	"warden": "Зенит N: в начале вашего хода ослепляет N случайных врагов на 1 ход.",
	"germinate": "Проращивание N: активка — потратьте 1 кристалл, призовите росток N/N. Раз в ход.",
	"harvest": "Жатва N: активка — потратьте 1 кристалл и пожертвуйте другое своё существо, этому +N/+N. Раз в ход.",
	...
	"renewal": "Проблеск N: при смерти возвращает N случайных существ из сброса в руку.",
	...
	"chill": "Стужа N: вражеские существа -N к атаке, пока это в игре.",
	"incandescence": "Накал N: ваши существа +N к атаке, пока это в игре.",
	"haze": "Дымка N: пока аура в игре, вражеские заклинания стоят на N дороже.",
	"birefringence": "Раздвоение: ваш нацеленный эффект (заклинание или батлкрай) бьёт ещё и вторую случайную цель.",
	"pinpoint": "Острый фокус: урон этой карты нельзя поглотить Щитом или Нимбом.",
	...
	"prowess": "Отклик N: каждое ваше заклинание даёт этому существу +N к атаке до конца хода.",
...
# When an effect fires. The engine runs any of these as a data trigger, so the
# card must print the timing -- an effect whose trigger has no label here would
# be rendered as a bare sentence (or, worse, dropped) and the player could not
# tell WHEN it happens. on_play is the exception: it is the default, printed as
# "При выходе" only for a body (a spell just prints its sentence).
const TRIGGER := {
	"on_play": "При выходе",
...
static func trigger_label(trigger: String) -> String:
	return String(TRIGGER.get(trigger, ""))


# Delay folds the timing into the effect's own "when": "Через 2 хода:".
static func delay_prefix(n: int) -> String:
	return _fix_plurals("Через N ход(ов)".replace("N", str(n)), n)
